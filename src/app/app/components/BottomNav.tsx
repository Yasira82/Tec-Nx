...
import { Icon, type NxIconName } from './Icon';
import { useTranslation } from '@/lib/i18n';

export type NxTab = 'board' | 'post' | 'pro' | 'settings';

export function BottomNav({ active, onSelect }: { active: NxTab; onSelect: (t: NxTab) => void }) {
  const { t } = useTranslation();
  const ITEMS: { key: NxTab; icon: NxIconName; label: string }[] = [
    { key: 'board',    icon: 'briefcase', label: t.nx.nav.board    },
    { key: 'post',     icon: 'plus',      label: t.nx.nav.post     },
    { key: 'pro',      icon: 'star',      label: t.nx.nav.pro      },
    { key: 'settings', icon: 'settings',  label: t.nx.nav.settings },
  ];
  return (
    <nav style={{
