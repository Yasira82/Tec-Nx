...
// NX Pro — the monetization surface (C-112/ADR-010 §7: featured postings +
// boosted visibility for opportunity posters). A real Pi User-to-App payment;
// also satisfies the Pi Portal "Process a Transaction" step. ADR-007 dual-mode:
// Hub navigation → Mode 1 (Hub modal); standalone → Mode 2 (createU2APayment).
// Approves under PI_API_KEY_NX (never the default Hub key — C-12 §11).
...
const PRICE   = 8;                         // π / month
const ITEM_ID = 'nx_pro_monthly';
const MEMO    = 'TEC NX — Pro (1 month)';
...
export function NxPro() {
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      setStatus('paying');
      const result = await createU2APayment(PRICE, MEMO, { item_id: ITEM_ID, plan: 'nx_pro' }, internalId);
  ...
  if (status === 'success') {
    return (
      <div style={{ ...card, borderColor: `${TEC_COLORS.success}66` }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: TEC_COLORS.success }}>✅ NX Pro active</div>
        <div style={{ fontSize: 12, color: TEC_COLORS.subtext, marginTop: 6 }}>
          Payment received. Featured postings + boosted visibility unlocked 🧩.
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={card}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 12 }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: TEC_COLORS.gold }}>🧩 NX Pro</div>
